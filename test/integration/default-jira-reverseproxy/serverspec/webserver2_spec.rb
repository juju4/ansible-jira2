...
describe command('curl -kvL http://localhost/'), :if => os[:family] == 'ubuntu' && os[:release] == '16.04' do
...
describe command('curl -kvL http://localhost/'), :if => os[:family] == 'ubuntu' && os[:release] == '14.04' do
#  its(:stdout) { should match /Apache2 Ubuntu Default Page: It works/ }
...
describe command('curl -kvL http://localhost/'), :if => os[:family] == 'redhat' do
